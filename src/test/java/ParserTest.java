...
""";
        ...
        PriceListFormatted priceListTable = (PriceListFormatted) parser.parseFromCsv(csvText);

        System.out.println("Loaded " + priceListTable.getRows().size() + " rows");

        PriceListRow first = (PriceListRow) priceListTable.getRows().get(0);
        System.out.println("First row: " + first.getWeight() + " kg, " + first.getVolume() + " m");
        System.out.println("BA1 price: " + first.getRegions().get("BA1"));


        String exported = parser.exportToCsv(priceListTable);
        System.out.println("\n=== EXPORTED CSV ===");
        String[] lines = exported.split("\n");
        for (int i = 0; i < lines.length; i++) {
            System.out.println(lines[i]);
        }
        System.out.println("total " + lines.length + " lines");
    }
}