...
public class DaoTest {
    ...
    void insertDuplicateNameFails() throws SQLException {
        userDAO.insert(new User("Bob", "Reed","bob@test.com", Role.USER));
        assertThrows(SQLException.class,
                () -> userDAO.insert(new User("Bob","Write", "other@test.com", Role.ADMIN)));
    }

    @Test
    @DisplayName("Role ordinal round-trips correctly for all roles")
    void allRolesRoundTrip() throws SQLException {
        userDAO.insert(new User("User1", "one", "u1@test.com", Role.USER));
        userDAO.insert(new User("User2", "two",  "u2@test.com", Role.POWER_USER));
        userDAO.insert(new User("User3", "three",  "u3@test.com", Role.ADMIN));
